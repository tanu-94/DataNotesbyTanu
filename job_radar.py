...
SMS_PHONE_NUMBER = os.getenv("ALERT_PHONE_NUMBER")
...
# ----------------- 3. DISPATCH SMS -----------------
def send_sms_alert(jobs):
    if not FAST2SMS_API_KEY or not SMS_PHONE_NUMBER:
        return

    # Fast2SMS quick format (160 character friendly)
    if jobs:
        top_job = jobs[0]
        sms_text = f"🎯 Job Radar Alert: {len(jobs)} new roles found in Bengaluru! Top: {top_job['title']} at {top_job['company']}. Check your email for apply links."
    else:
        sms_text = "🎯 Job Radar: Fresh daily sweeps ready on Naukri & LinkedIn. Check your email for one-click apply links."

    url = "https://www.fast2sms.com/dev/bulkV2"
    payload = {
        "route": "q",
        "message": sms_text,
        "numbers": SMS_PHONE_NUMBER
    }
    headers = {
